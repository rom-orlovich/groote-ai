...
const mapTrendData = (data: TrendApiResponse | null, days: number): DailyMetric[] => {
...
const mapAgentData = (data: AgentApiResponse | null): AgentMetric[] => {
  ...
  const { data: trendData, isLoading: isTrendLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `/api/analytics/costs/histogram?days=${days}&granularity=${granularity}`,
      );
