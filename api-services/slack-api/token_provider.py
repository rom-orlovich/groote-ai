...
class TokenProvider:
    ...
    async def get_token(self) -> str:
        ...
        if self._static_token:
            logger.debug("static_token_used", platform="slack")
            return self._static_token

        raise RuntimeError("No Slack token available (OAuth unreachable, no static token)")

    async def _fetch_oauth_token(self) -> str | None:
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(
                    f"{self._oauth_url}/internal/token/slack",
                    headers={"X-Internal-Service-Key": self._service_key},
                )
                if response.status_code == 200:
                    data: dict[str, Any] = response.json()
                    if data.get("available") and data.get("token"):
                        self._set_cache("slack", data["token"])
                        logger.info("oauth_token_retrieved", platform="slack")
                        return data["token"]
        except httpx.RequestError as e:
            logger.warning("oauth_service_unreachable", platform="slack", error=str(e))
        return None
